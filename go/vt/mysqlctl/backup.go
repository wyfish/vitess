...
const (
	// subordinateStartDeadline is the deadline for starting a subordinate
	subordinateStartDeadline = 30
)
...
func Restore(ctx context.Context, params RestoreParams) (*BackupManifest, error) {
	...
	if len(bhs) == 0 {
		...
		if err := params.Mysqld.ResetReplication(ctx); err != nil {
			params.Logger.Errorf("error resetting subordinate replication: %v. Continuing", err)
		}
