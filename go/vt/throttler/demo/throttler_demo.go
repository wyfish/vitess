...
// rate than mains and fall behind at high write throughput.
// (Mostly they fall behind because MySQL replication is single threaded but
//  the write throughput on the main does not have to.)
//
// This demo simulates a client (writer), a main and a replica.
// The client writes to the main which in turn replicas everything to the
// replica.
// The replica measures its replication lag via the timestamp which is part of
// each message.
// While the main has no rate limit, the replica is limited to
...
// main simulates an *unthrottled* MySQL main which replicates every
...
func (m *main) execute(msg time.Time) {
	m.replica.replicate(msg)
}

// replica simulates a *throttled* MySQL replica.
// If it cannot keep up with applying the main writes, it will report a
// replication lag > 0 seconds.
type replica struct {
	fakeTablet *testlib.FakeTablet
	qs         *fakes.StreamHealthQueryService

	// replicationStream is the incoming stream of messages from the main.
...
func (r *replica) processReplicationStream() {
	...
	for msg := range r.replicationStream {
		...
		if now.Sub(r.lastHealthUpdate) > r.lagUpdateInterval {
			...
			r.qs.AddHealthResponseWithSecondsBehindMain(lagTruncated)
...
func newClient(main *main, replica *replica) *client {
...
func main() {
	...
	main := &main{replica: replica}
	client := newClient(main, replica)
	client.run()
