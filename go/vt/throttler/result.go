...
alias: {{.Alias}} lag: {{.LagRecordNow.Stats.SecondsBehindMain}}s
...
lag before: {{.LagBefore}} ({{.AgeOfBeforeLag}} ago) rates (main/subordinate): {{.MainRate}}/{{.GuessedSubordinateRate}} backlog (old/new): {{.GuessedSubordinateBacklogOld}}/{{.GuessedSubordinateBacklogNew}}
reason: {{.Reason}}`))
...
type result struct {
	...
	MainRate             int64
	GuessedSubordinateRate       int64
	GuessedSubordinateBacklogOld int
	GuessedSubordinateBacklogNew int
}
...
func (r result) LagBefore() string {
	...
	return fmt.Sprintf("%ds", r.LagRecordBefore.Stats.SecondsBehindMain)
}
