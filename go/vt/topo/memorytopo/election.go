...
func (c *Conn) NewMainParticipation(name, id string) (topo.MainParticipation, error) {
	c.factory.mu.Lock()
...
// cMainParticipation implements topo.MainParticipation.
//
...
// wins the election.
type cMainParticipation struct {
...
func (mp *cMainParticipation) WaitForMainship() (context.Context, error) {
...
func (mp *cMainParticipation) Stop() {
	close(mp.stop)
	<-mp.done
}

// GetCurrentMainID is part of the topo.MainParticipation interface
func (mp *cMainParticipation) GetCurrentMainID(ctx context.Context) (string, error) {
	electionPath := path.Join(electionsPath, mp.name)
