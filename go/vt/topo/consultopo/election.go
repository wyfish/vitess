...
func (s *Server) NewMainParticipation(name, id string) (topo.MainParticipation, error) {
	return &consulMainParticipation{
		s:    s,
		name: name,
		id:   id,
...
// consulMainParticipation implements topo.MainParticipation.
//
// We use a key with name <global>/elections/<name> for the lock,
// that contains the id.
type consulMainParticipation struct {
...
func (mp *consulMainParticipation) WaitForMainship() (context.Context, error) {
	...
	// We have the lock, keep mainship until we lose it.
	...
	go func() {
		...
		case <-lost:
			...
			if err := l.Unlock(); err != nil {
				log.Errorf("main election(%v) Unlock failed: %v", mp.name, err)
			}
		case <-mp.stop:
			// Stop was called. We stop the context first,
			// so the running process is not thinking it
			// is the main any more, then we unlock.
			lockCancel()
			if err := l.Unlock(); err != nil {
				log.Errorf("main election(%v) Unlock failed: %v", mp.name, err)
			}
...
func (mp *consulMainParticipation) Stop() {
	close(mp.stop)
	<-mp.done
}

// GetCurrentMainID is part of the topo.MainParticipation interface
func (mp *consulMainParticipation) GetCurrentMainID(ctx context.Context) (string, error) {
	electionPath := path.Join(mp.s.root, electionsPath, mp.name)
