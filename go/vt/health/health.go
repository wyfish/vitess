...
var (
	...
	// ErrSubordinateNotRunning is returned by health plugins when replication
	...
	ErrSubordinateNotRunning = errors.New("subordinate is not running")
)
...
type Reporter interface {
	...
	// it is a subordinate type or not, and that its query service
	// should be running or not. If Report returns an error it
	// implies that the tablet is in a bad shape and not able to
	// handle queries.
	Report(isSubordinateType, shouldQueryServiceBeRunning bool) (replicationDelay time.Duration, err error)
...
func (fc FunctionReporter) Report(isSubordinateType, shouldQueryServiceBeRunning bool) (time.Duration, error) {
	return fc(isSubordinateType, shouldQueryServiceBeRunning)
}
...
func (ag *Aggregator) Report(isSubordinateType, shouldQueryServiceBeRunning bool) (time.Duration, error) {
	wg := sync.WaitGroup{}
	...
	for name, rep := range ag.reporters {
		...
		go func(index int, name string, rep Reporter) {
			...
			results[index].delay, results[index].err = rep.Report(isSubordinateType, shouldQueryServiceBeRunning)
		}(index, name, rep)
		index++
	}
	ag.mu.Unlock()
	...
	for _, s := range results {
		...
		case ErrSubordinateNotRunning:
			// Return the ErrSubordinateNotRunning sentinel
			// value, only if there are no other errors.
			err = ErrSubordinateNotRunning
		case nil:
			if s.delay > result {
				result = s.delay
			}
