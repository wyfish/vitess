...
class ShardTablets(namedtuple('ShardTablets', 'main replicas rdonlys')):
  """ShardTablets is a container for all the tablet.Tablets of a shard.

  `main` should be a single Tablet, while `replicas` and `rdonlys` should be
  ...
  def all_tablets(self):
    ...
    """
    return [self.main] + self.replicas + self.rdonlys
...
all_shard_tablets = ShardTablets(shard_main, [shard_replica], [shard_rdonly1])
shard_0_tablets = ShardTablets(
    shard_0_main, [shard_0_replica], [shard_0_rdonly1])
shard_1_tablets = ShardTablets(
    shard_1_main, [shard_1_replica], [shard_1_rdonly1])
...
class TestBaseSplitClone(unittest.TestCase, base_sharding.BaseShardingTest):
  ...
  def run_shard_tablets(
    ...
    This encompasses the following steps:
      ...
      4. Force reparent to the main tablet
    ...
    # Start tablets.
    #
    # NOTE: The future main has to be started with type 'replica'.
    shard_tablets.main.start_vttablet(
    ...
    # All tables should be NOT_SERVING until we run InitShardMain.
    for t in shard_tablets.all_tablets:
      t.wait_for_vttablet_state('NOT_SERVING')

    # Reparent to choose an initial main and enable replication.
    utils.run_vtctl(
        ['InitShardMain', '-force', 'test_keyspace/%s' % shard_name,
         shard_tablets.main.tablet_alias], auto_log=True)
  ...
  def run_split_diff(self, keyspace_shard, source_tablets, destination_tablets):
    ...
    Sets all former rdonly subordinates back to rdonly.
  ...
  def setUp(self):
    ...
    try:
      ...
      self.insert_values(shard_main, self.num_insert_rows, 2)
      logging.debug(
          'Done inserting initial data, waiting for replication to catch up')
      utils.wait_for_replication_pos(shard_main, shard_rdonly1)
  ...
  def tearDown(self):
    ...
    for shard_tablet in [all_shard_tablets, shard_0_tablets, shard_1_tablets]:
      for t in shard_tablet.all_tablets:
        ...
        # (the main tablets after an emergency reparent)
        utils.run_vtctl(['DeleteTablet', '-allow_main', t.tablet_alias],
...
class TestBaseSplitCloneResiliency(TestBaseSplitClone):
  ...
  def verify_successful_worker_copy_with_reparent(self, mysql_down=False):
    ...
    2. Wait until the worker successfully resolves the destination mains.
    ...
    Args:
      mysql_down: boolean. If True, we take down the MySQL instances on the
        destination mains at first, then bring them back and reparent away.
    ...
    #                destination shards are not behind the main's replication
    #                position.
    args = ['SplitClone',
            '--offline=false',
    ...
    if mysql_down:
      ...
      logging.debug('Worker is in copy state, Shutting down mysqld on destination mains.')
      utils.wait_procs(
          [shard_0_main.shutdown_mysql(),
           shard_1_main.shutdown_mysql()])
      ...
      # Bring back mains. Since we test with semi-sync now, we need at least
      # one replica for the new main. This test is already quite expensive,
      # so we bring back the old main as a replica rather than having a third
      # replica up the whole time.
      logging.debug('Restarting mysqld on destination mains')
      utils.wait_procs(
          [shard_0_main.start_mysql(),
           shard_1_main.start_mysql()])

      # Reparent away from the old mains.
      utils.run_vtctl(
          ['PlannedReparentShard', '-keyspace_shard', 'test_keyspace/-80',
           '-new_main', shard_0_replica.tablet_alias], auto_log=True)
      utils.run_vtctl(
          ['PlannedReparentShard', '-keyspace_shard', 'test_keyspace/80-',
           '-new_main', shard_1_replica.tablet_alias], auto_log=True)
      ...
      # Make sure that vtworker got past the point where it picked a main
    ...
    self.assertGreater(worker_vars['WorkerRetryCount'], 1,
                       ...
                       " destination mains at least once, but it didn't")
    ...
    self.assert_shard_data_equal(0, shard_main, shard_0_tablets.replica)
    self.assert_shard_data_equal(1, shard_main, shard_1_tablets.replica)
...
class TestMysqlDownDuringWorkerCopy(TestBaseSplitCloneResiliency):
  ...
  def test_mysql_down_during_worker_copy(self):
    """This test simulates MySQL being down on the destination mains."""
