...
all_tablets = [shard_main, shard_replica, shard_rdonly1,
...
index by_msg (msg)
  ...
  def _insert_startup_values(self):
    self._insert_startup_value(shard_main, 'resharding1', 1, 'msg1')
    self._insert_startup_value(shard_main, 'resharding1', 2, 'msg2')
    self._insert_startup_value(shard_main, 'resharding1', 3, 'msg3')
  ...
  def _check_startup_values(self):
    ...
    for t in [shard_0_main, shard_0_replica, shard_0_rdonly1]:
      self._check_value(t, 'resharding1', 1, 'msg1', 0x1000000000000000)
    for t in [shard_1_main, shard_1_replica, shard_1_rdonly1]:
  ...
  def _insert_lots(self, count, base=0):
    for i in xrange(count):
      self._insert_value(shard_main, 'resharding1', 10000 + base + i,
                         'msg-range1-%d' % i, 0xA000000000000000 + base + i)
      self._insert_value(shard_main, 'resharding1', 20000 + base + i,
  ...
  def test_resharding(self):
    ...
    for t in [shard_main, shard_replica, shard_rdonly1]:
      t.create_db('vt_test_keyspace')

    # replica is not started, InitShardMain should timeout
    ...
    for t in [shard_main, shard_rdonly1]:
      t.wait_for_vttablet_state('NOT_SERVING')

    # reparent to make the tablets work - expect fail
    # because replica tablet is not up
    _, stderr = utils.run_vtctl(['InitShardMain', '-force', 'test_keyspace/0',
                     shard_main.tablet_alias], auto_log=True, expect_fail=True)
    ...
    utils.VtGate().start(cache_ttl='0', tablets=[
        shard_main, shard_replica, shard_rdonly1])
    ...
    self._backfill_keyspace_id(shard_main)
    ...
    sharded_tablets = [shard_0_main, shard_0_replica, shard_0_rdonly1,
                       shard_1_main, shard_1_replica, shard_1_rdonly1]
    ...
    utils.vtgate.wait_for_endpoints('test_keyspace.0.main', 1, var=var)
    ...
    utils.vtgate.wait_for_endpoints('test_keyspace.-80.main', 1, var=var)
    ...
    self.check_destination_main(shard_0_main, ['test_keyspace/0'])
    self.check_destination_main(shard_1_main, ['test_keyspace/0'])
    ...
    self.check_binlog_player_vars(shard_0_main, ['test_keyspace/0'],
                                  seconds_behind_main_max=30)
    self.check_binlog_player_vars(shard_1_main, ['test_keyspace/0'],
                                  seconds_behind_main_max=30)
    self.check_binlog_server_vars(shard_replica, horizontal=True,
    ...
    tablet.kill_tablets([shard_main, shard_replica, shard_rdonly1])
    ...
    tablet.kill_tablets([shard_0_main, shard_0_replica, shard_0_rdonly1,
                         shard_1_main, shard_1_replica, shard_1_rdonly1])
